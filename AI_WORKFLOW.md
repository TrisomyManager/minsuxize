...
`minsuxize` is an ASP.NET Core MVC prototype for recording, reviewing, and publishing Chinese local folklore practices.
...
- Repository status: prototype with real browsing flow and admin review flow
...
1. `src/MinsuXize.Web/Program.cs`
2. `src/MinsuXize.Web/Services/IFolkloreRepository.cs`
3. `src/MinsuXize.Web/Data/AppDbContext.cs`
4. `src/MinsuXize.Web/Controllers/*.cs`
5. `src/MinsuXize.Web/Models/*.cs`
6. `src/MinsuXize.Web/ViewModels/*.cs`
7. `src/MinsuXize.Web/Views/*`
8. `src/MinsuXize.Web/Data/Seed/DbSeeder.cs`
...
- This document is the primary AI-facing structure guide
