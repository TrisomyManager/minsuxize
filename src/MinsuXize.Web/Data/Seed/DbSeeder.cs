    ...
    private static void SeedFestivals(AppDbContext context)
    {
        UpsertFestival(
            ...
            new FestivalEntity
            {
                ...
                Category = "岁时节日",
                LunarLabel = "农历正月初一",
                Summary = "年度最重要的传统节日，涵盖除夕祭祖、年夜饭、拜年、红包、舞龙舞狮等丰富仪式。",
                CoreTopicsJson = JsonListSerializer.Serialize("祭祖", "年夜饭", "拜年", "红包", "舞龙舞狮")
            });
    ...
    private static void SeedEntries(AppDbContext context)
    {
        ...
        UpsertEntry(
            ...
            new FolkloreEntryEntity
            {
                ...
                RegionId = 6,
                FestivalId = 1,
                Summary = "除夕傍晚在堂屋或院中设供桌，通过焚香、献供、叩拜、念祷等流程，完成年度祭祖仪式。",
                HistoricalOrigin = "源于周代宗庙祭祀传统，融合了佛教香供养与道教祭祖仪式，在明清时期定型为家庭祭祖。",
                SymbolicMeaning = "通过祭祖强化家族认同，祈求祖先庇佑，并作为年度时间节点的标志。",
                InheritanceStatus = "仍被部分家庭保留，但年轻一代参与度下降。",
                ChangeNotes = "供品从传统三牲简化为水果、糕点；祭词从固定文本变为自由表达。",
                OralText = "祖先在上，子孙某某，今值岁末，特备香烛供品，恭请祖先享用，保佑全家平安，来年丰收。",
